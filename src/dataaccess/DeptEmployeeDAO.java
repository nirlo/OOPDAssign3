...
public class DeptEmployeeDAO implements DAO<DeptEmployee> {
	...
	public DeptEmployeeDAO() {
		...
		factory = (Factory<DeptEmployee>) DTOFactoryCreator.getFactory(DeptEmployee.class);
	}
	...
	public DeptEmployee getByID(String id) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			deptEmp = (DeptEmployee) factory.createListFromResultSet(rs);
		}catch(SQLException ex) {
			Logger.getLogger(EmployeeDAO.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
